...
def parse_args():
    ...
    parser.add_argument('--path2data', type=str, default='C:\\Users\\aksha\Documents\\Thesis\\Pupil_right_3\\',
                        help='path to eye videos')
    parser.add_argument('--save_maps', type=int, default=1,
                        help='save segmentation maps')
    parser.add_argument('--save_overlay', type=int, default=1,
                        help='save output overlay')
    parser.add_argument('--vid_ext', type=str, default='avi',
...
def evaluate_ellseg_per_video(path_vid, args, model):
    ...
    while ret:
        if counter%3==0:
            try:
                ...
                # print("pupil ---",pupil_ellipse)
                
                r1.append(pupil_ellipse[2])
                r2.append(pupil_ellipse[3])
            ...
            pbar.update(1)
            counter+=1
    # print("finisshsssssssssssssssssssssssssshhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh")
    print(full_file_name.split(".")[0])
    import json
    ...
    #     r['r1']=r1
    #     r['r2']=r2
    # json_decoded[file_name] = r

    # with open("X:\\Python_projects\\Eye tracking\\pupil_radius.json", 'w') as json_file:
...
if __name__=='__main__':
    ...
    netDict = torch.load(args.loadfile, map_location=device)

    model = model_dict['ritnet_v4']
    ...
    s = time.time()
    #%% Read in each video
    path_obj = [Path(args.path2data)]
    files = [file for file in os.listdir(Path(args.path2data)) if file.endswith(".avi")]
    # files= [  'Rishi_pupil_left.avi', 'Ritwik_pupil_left.avi', 'Ruchir_pupil_left.avi', 'Sakthi_Prasasd_pupil_left.avi', 
    # 'Sidhu_pupil_left.avi', 'Soumya_pupil_left.avi', 'Steve_Thomas_pupil_left.avi', 'Swetha_pupil_left.avi', 'Vignesh_pupil_left.avi', 
    # 'Vijay_Anand_pupil_left.avi', 'Yash_pupil_left.avi', 'Yogi_pupil_left.avi', 'Yuki_pupil_left.avi', 'Zahra_pupil_left.avi']
    # ['Adidev_pupil2_left.avi', 'Akash_pupil_left.avi', 'Alwin_pupil_left.avi', 'Amit_pupil_left.avi', 'Avinash_pupil_left.avi', 
    # 'Ayushi_pupil_left.avi', 'Devika_pupil_left.avi', 'Dimple_pupil_left.avi', 'Kaushal_pupil_left.avi','Keerti_pupil_left.avi', 
    # 'Koundinya_pupil_left.avi', 'Kunal_Rane_pupil_left.avi', 'Lokesh_pupil_left.avi', 'Mangesh_pupil_left.avi', 'Manish_pupil_left.avi', 
    # 'Nivetha_Suresh_pupil_left.avi', 'Pramod_pupil_left.avi', 'Pritish_pupil_left.avi', 'Rahi_pupil_left.avi', 'Rex_pupil_left.avi', 
    # 'Rhutujit_pupil_left.avi',]
    print(files)
    # print(path_obj)
    for path_vid in files:
        print(args.path2data + str(path_vid))
        if '_ellseg' not in str(path_vid):
            
            evaluate_ellseg_per_video(args.path2data + str(path_vid), args, model)
    f = time.time()
    print("total time taken in seconds ----",f-s)
